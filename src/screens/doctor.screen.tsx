...
const Doctor = () => {
  ...
  const handleMessageChange =(index:number , e: React.ChangeEvent<HTMLInputElement>) =>{
    ...
    <div className="flex flex-col space-y-6 p-6 bg-sky-50 min-h-screen items-center">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between w-full max-w-5xl gap-4">
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          />
          <select value={statusFilter}   onChange={(e) => setStatusFilter(e.target.value)} className="p-3 border-2 border-gray-300 rounded-lg shadow-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
  ...
  <div className="mt-6 w-full max-w-5xl">
    {filteredByStatus.map((appointment, index) => (
      <div key={appointment.id || index}className="p-6 bg-gradient-to-r from-white to-blue-50 rounded-xl shadow-lg mb-6 border border-gray-200 hover:shadow-2xl transition duration-300 ease-in-out">
        <h3 className="text-2xl font-semibold text-blue-700 mb-4">Appointment Details</h3>
        <div className="space-y-3 text-gray-800 text-lg">
          <p><strong>Name:</strong> {appointment.userName || "Not Set"}</p>
          <p><strong>Phone:</strong> {appointment.patientPhone || "Not Set"}</p>
          <p><strong>Age:</strong> {appointment.age || "Not Set"}</p>
          <p><strong>Gender:</strong> {appointment.gender || "Not Set"}</p>
          <p><strong>Reason:</strong> {appointment.reason || "Not Set"}</p>
          <p><strong>Department:</strong> {appointment.department || "Not Set"}</p>
          <p><strong>Date:</strong> {appointment.pickDate || "Not Set"}</p>
          <p><strong>Time:</strong> {appointment.pickTime || "Not Set"}</p>
          <p><strong>Status:</strong> 
            <span>
              {appointment.status || "Not Set"}
            </span>
          </p>
          <p><strong>Message:</strong> {appointment.message || "Not Set"}</p>
        </div>
        ...
        <p className="text-gray-500 text-xl text-center mt-10">No appointments found with that name or status.</p>
       )}
    </div>
  );
};
